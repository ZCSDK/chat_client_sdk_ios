...
// 根据导航栏颜色修改图片的黑色 和导航栏上文字的黑白
-(void)createVCTitleViewisBlack:(BOOL)isBlack;
/// 仅创建自定义导航栏， 后期不再使用，直接使用createVCTitleView
-(void)createCustomTitleView;
/// 仅创建自定义导航栏 并设置导航栏文字颜色 黑白
-(void)createCustomTitleViewisBlack:(BOOL)isBlack;
// 系统导航栏上的文字颜色  非黑即白
-(void)updateNavOrTopViewisBlack:(BOOL)isBlack;
// 仅设置自定义导航，1.0.2之后废弃
...
/// 更新系统导航栏的按钮图片和文字颜色
-(void)setLeftTags:(NSArray *__nullable)leftTags rightTags:(NSArray *__nullable)rightTags isBlack:(BOOL)isBlack;
/// V7新增方法 更新导航栏 是否黑色颜色文案
-(void)setLeftTags:(NSArray *__nullable)leftTags rightTags:(NSArray *__nullable)rightTags titleView:(UIView * _Nullable)titleView isBlack:(BOOL)isBlack;
